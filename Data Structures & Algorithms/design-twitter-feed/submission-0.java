...
class Twitter {
    ...
    public List<Integer> getNewsFeed(int userId) {
        ...
        Set<Integer> followees = followeesMap.containsKey(userId) ? followeesMap.get(userId) : new HashSet<>();
        ...
        for(Integer followeeId : followees){
            if(userTweetMap.containsKey(followeeId) && userTweetMap.get(followeeId).size() > 0){
    ...
    public void unfollow(int followerId, int followeeId) {
        boolean status = followeesMap.get(followerId).remove(followeeId);
        if(status){
            // success is not printed because assumption is test cases will all succeed
        } else {
