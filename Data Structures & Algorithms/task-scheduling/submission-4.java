...
class Solution {
    public int leastInterval(char[] tasks, int n) {
        ...
        createActiveTasks(tasks, activeTasks);
        ...
        while(!activeTasks.isEmpty() || !readyTasks.isEmpty()){
            ...
            if(!activeTasks.isEmpty()){
                ...
                if(task.freq > 0){
                    // sent for cooldown
                    task.availableAt = currCycle + n + 1;
                    readyTasks.add(task);
                }
            }
    ...
    private void createActiveTasks(char[] tasks, PriorityQueue<Task> activeTasks){
        // created this locally inside child function so memory can be claimed back early
        int[] freq = new int[26];
