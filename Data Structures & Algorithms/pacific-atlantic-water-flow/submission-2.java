class Solution {
    ...
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        ...
        for(int i = 0; i < heights.length; i++){
            dfs(result, heights, i, 0, atl, pac, false);
            dfs(result, heights, i, heights[0].length - 1, atl, pac, true);
        }

        for(int j = 0; j < heights[0].length; j++){
            dfs(result, heights, 0, j, atl, pac, false);
            dfs(result, heights, heights.length - 1, j, atl, pac, true);
        }
